...
export default function ConfiguracionBotTab({ addToast }) {
  ...
  const [, setTick]                           = useState(0); // para re-render del timer
  ...
  useEffect(() => {
    async function cargarConfig() {
      ...
      if (error) {
        addToast('⚠️ Error al cargar la configuración de Bruno');
        return;
      }
      if (data) {
  ...
  // Actualiza el texto "hace X minutos" cada 30s
  useEffect(() => {
    if (!lastSaved) return;
      ...
      <div className="card-grid" style={{ marginBottom: 28 }}>
        <div className="stat-card" style={{ borderLeft: `4px solid ${botEncendido ? 'var(--status-active-text)' : 'var(--status-paused-text)'}` }}>
          ...
          <span
            ...
            style={{
              fontSize: 18,
              color: botEncendido ? 'var(--status-active-text)' : 'var(--status-paused-text)',
            }}
          >
          ...
          <span className="stat-desc">
            {botEncendido
              ? 'Bruno responde mensajes automáticamente'
              : 'Bruno ignora todos los mensajes entrantes'}
          </span>
        </div>
        ...
        <div className="stat-card" style={{ borderLeft: `4px solid ${reservasActivas ? 'var(--status-active-text)' : 'var(--status-paused-text)'}` }}>
          ...
          <span
            ...
            style={{
              fontSize: 18,
              color: reservasActivas ? 'var(--status-active-text)' : 'var(--status-paused-text)',
            }}
          >
          ...
          <span className="stat-desc">
            {reservasActivas
              ? 'Bruno acepta y gestiona reservas'
              : 'Bruno avisa que hoy no hay reservas disponibles'}
          </span>
        </div>
      ...
      <form onSubmit={handleSave}>
        ...
        <div className="form-section">
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px 20px', background: 'var(--bg-primary)', border: '1px solid var(--border-color)', borderRadius: 12, marginBottom: 12 }}>
            ...
            <label className="switch" style={{ flexShrink: 0, marginLeft: 24 }}>
              <input
                type="checkbox"
                checked={botEncendido}
                onChange={e => setBotEncendido(e.target.checked)}
              />
              <span className="slider"></span>
            </label>
          </div>

          {/* Toggle: reservas_activas */}
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px 20px', background: 'var(--bg-primary)', border: '1px solid var(--border-color)', borderRadius: 12 }}>
            ...
            <label className="switch" style={{ flexShrink: 0, marginLeft: 24 }}>
              <input
                type="checkbox"
                checked={reservasActivas}
                onChange={e => setReservasActivas(e.target.checked)}
              />
              <span className="slider"></span>
            </label>
          </div>
        ...
        <div className="form-section">
          ...
          <p className="form-section-desc">
            ...
            pida ver la carta o confirme una reserva. Se actualiza todos los días según la disponibilidad.
          </p>
